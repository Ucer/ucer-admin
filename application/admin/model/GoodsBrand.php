...
class GoodsBrand extends Model{
    ...
    public function getPageList($where=1,$per=20,$field="*",$order_type='asc',$order='created_at'){
        ...
        if($group_list){
            foreach($group_list as $k=>$v){
                $v['goods_category_id'] = Db::name('goods_category')->where(['id'=>$v['goods_category_id']])->value('cat_name');
    ...
    public function handle($data,$id=0)
    {
        ...
        $rules =  [
            ['name','unique:goods_brand','品牌已经存在'],
        ];
        if($id >0){
            ...
            try{
                ...
                if(false ===$result){
                    ...
                    adminLog("修改商品品牌[".$data['name']."]",req('url'));
                    return ['code'=>1,'data'=>'','msg'=>'商品品牌修改成功'];
                }
            ...
            try{
                ...
                if(false ===$result){
                    ...
                    adminLog("添加商品品牌[".$data['name']."]",req('url'));
                    return ['code'=>1,'data'=>'','msg'=>'商品品牌添加成功'];
                }
            }catch(\PDOException $e){
                return ['code'=>0,'data'=>'','msg'=>$e->getMessage()];
            }
    ...
    {
        //检查商品规格
        $sun_spec = Db::name('goods')->where(['goods_brand_id'=>$ids])->find();
        if($sun_spec){
